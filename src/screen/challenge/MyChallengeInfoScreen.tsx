...
const MyChallengeInfoScreen = () => {
    ...
    const {myChallengeList, renderMyChallenge, bankList} = store.challengeState(state => state)
    ...
    useEffect(() => {
        ...
        if(queueSequence[0] == "room?GET") {
            ...
            console.log("test", hookQueue[0]?.response?.data?.data)
            console.log("test1", myChallengeList.filter(item => Number(item.roomId) == Number(selectedId))[0])
            store.hookState.setState({hookQueue: [], queueSequence: []});
        } else if (queueSequence[0] === "room/user-list?GET") {
    ...
    useEffect(() => {
        ...
        if (renderMyChallenge[0]?.idx) {
            Welspy.challenge.getChallengeUserList(1, 4, renderMyChallenge[0].idx)
        }
    }, [renderMyChallenge]);
    ...
    useEffect(() => {
        if(bankList[0]) {
            setDmBalance(bankList.map(item => item.balance))
        }
    }, [bankList]);
    ...
    return (
        <>
            {/*<Image src={renderItem?.imageUrl} style={{width: Width, height: Height/4, alignSelf: 'center'}} resizeMode="cover" />*/}
                    ...
                    renderItem?.idx ?
                        <ScrollView showsVerticalScrollIndicator={false}>
                            <View style={styles.scrollContainer}>
                                <DismissButton style={{height: Height/13.5, justifyContent: "flex-start", marginTop: -10, width: Width, alignSelf: 'center', paddingHorizontal: "5%"}} onPress={() => {store.challengeState.setState({renderMyChallenge: [{},{}]});setSelectedId(0);}}></DismissButton>
                                <View style={{width: "100%"}}>
                                    ...
                                    <View style={{width: "80%", height: Height/43.5, flexDirection: "row"}}>
                                        ...
                                        <Text style={{fontSize: Width/28, fontWeight: '300', marginTop: 2.25}}> / {renderItem?.title?.split("|//+**+//|")[2]}</Text>
                                    </View>
                                </View>
                                <View style={[styles.userContainer, {marginTop: -12.5}]}>
                                    ...
                                    <View style={{alignSelf: 'center', alignItems: "center"}}>
                                        <Text style={{fontSize: Width/16, fontWeight: '600', marginBottom: 20, color: "#538eff"}}> {Number(Number(renderItem?.goalAmount) - (Number(renderMyChallenge[1]?.balance))).toLocaleString()}원 </Text>
                                    </View>
                                    <View onLayout={() => {LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);}} style={{marginBottom: "10%"}}>
                                        <CircleGraph
                                            radius={Width/4.25}
                                            strokeWidth={Width/17.5}
                                            percentage={dmBalance ? (dmBalance.reduce((a,b) => a+b) / Number(renderItem?.goalAmount)) * 100 : 100}
                                            color1={"#58b9ff"}
                                            color2={"#357bff"}
                                        />
                                        {/*<Text style={{fontSize: Width/12.5, fontWeight: "600", alignSelf: 'center', marginTop: Height/10, position: 'absolute'}}>{0}%</Text>*/}
                                    </View>
                                    <Text style={{position: "absolute", alignSelf: 'center', marginTop: 190, fontSize: 24}}>{Math.round((Number(dmBalance.reduce((a,b) => a+b)) / Number(renderItem?.goalAmount)) * 100)}%</Text>
                                        ...
                                        <View style={{flexDirection: 'row', width: '100%', justifyContent: 'space-between', height: Height/15}}>
                                            ...
                                            <View style={{alignItems: "flex-start", width: "83%", justifyContent: "center"}}>
                                                <View style={{flexDirection: 'row', justifyContent: "space-between", width: "100%", paddingHorizontal: 1.5}}>
                                                    <Text>{item.name}</Text>
                                                    <Text style={{marginRight: 4}}>{Math.round(Number(dmBalance.reduce((a,b) => a+b)) / Number(renderItem?.goalAmount) * 100)}%</Text>
                                                </View>
                                                <View style={{width: "100%", backgroundColor: "#e4e4e4", height: "35%", borderRadius: Width/50, overflow: 'hidden', marginTop: "1.5%"}}>
                                                    <View style={{width: `${(dmBalance.reduce((a,b) => a+b) / Number(renderItem?.goalAmount) * 100)}%`, height: "100%", backgroundColor: "rgba(83,142,255,0.64)", alignItems: 'flex-end', justifyContent: 'center'}}>
                                ...
                                myChallengeList.length == 0 ? <>
                                    ...
                                    <ScrollView style={{overflow: "hidden"}} contentContainerStyle={{paddingBottom: 80}}>
                                        ...
                                        <FlatList scrollEnabled={false} style={{marginTop: 30}} data={myChallengeList} renderItem={({item}) => (
                                            <TouchableOpacity onPress={() => setSelectedId(Number(item.roomId))} style={styles.selectorContainer}>
                                                ...
                                                <View style={{flexDirection: 'row', justifyContent: 'space-between', marginTop: 9}}>
                                                    <View style={{width: "90%", backgroundColor: "#e4e4e4", height: "25%", borderRadius: Width/50, overflow: 'hidden'}}>
                                                        <View style={{width: `${(Number(dmBalance.reduce((a,b) => a+b)) / Number(item?.goalAmount) * 100)}%`, height: "100%", backgroundColor: "rgba(83,142,255,0.64)", alignItems: 'flex-end', justifyContent: 'center'}}>
                                                        </View>
                                                    </View>
                                                    <Text style={{fontSize: 12, marginTop: -5}}>{Math.round(Number(dmBalance) / Number(item?.goalAmount) * 100)}%</Text>
                                                </View>
                                            </TouchableOpacity>
                                        )} />
                                    </ScrollView>
